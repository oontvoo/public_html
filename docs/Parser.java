...
public class Parser {
    ...
    private static final EnumSet<TokenKind> unsupported
        = EnumSet.of(FLOAT_LITERAL, LONG_LITERAL, DOUBLE_LITERAL);
    ...
    private boolean haveUnsupported()
    {
        TokenInfo tk = scanner.token();
        if (unsupported.contains(tk.kind()))
        {
            reportParserError("%s is not supported yet: %s - base = %s",
                              tk.kind(),
                              tk.image(),
                              tk.getBase());
            scanner.next();
    ...
    private JExpression literal() {
        ...
        if (haveUnsupported())                // some token are not parsed right now
            return new JWildExpression(line); // but check to make they are *scanned* correctly
        else if (have(INT_LITERAL)) {
            return new JLiteralInt(line, scanner.previousToken().image(), scanner.previousToken().getBase());
        } 
    ...
    private static void addDefaultImports(List<TypeName> imports)
    {
        /**
         * find all TypeName that are in <code>defaultImports</code>
         * but not in <code>imports</code>
         * Because we do not want to import a package twice
         */
        Set<TypeName> importSet = new HashSet<TypeName>(defaultImports);
        importSet.removeAll(imports);
        imports.addAll(importSet);
    }
    
    private static Set<TypeName> defaultImports
        = generateDefault();
    
    private static Set<TypeName> generateDefault()
    {
        // TODO: use reflection to collect all classes
        // in a given package (ie., java.lang.*)
        // For now, just manually add some commonly used classes
        
        Set<TypeName> ret = new HashSet<TypeName>();

        ret.add(new TypeName(0, "java.lang.UnsupportedOperationException"));
        ret.add(new TypeName(0, "java.lang.System"));
        ret.add(new TypeName(0, "java.lang.Boolean"));
        ret.add(new TypeName(0, "java.lang.Character"));
        ret.add(new TypeName(0, "java.lang.Integer"));
        ret.add(new TypeName(0, "java.lang.Long"));
        ret.add(new TypeName(0, "java.lang.Exception"));
        ret.add(new TypeName(0, "java.lang.Math"));
        ret.add(new TypeName(0, "java.lang.NullPointerException"));
